...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (index != 1 && min(index, parentIndex(index)) == index) {
    ...
    private void sink(int index) {
        ...
        if (inBounds(small)) {
            if (small < size && min(small, small + 1) == small + 1) {
                small++;
            }
            if (min(index, small) == index) {
                return;
            }
            swap(index, small);
            sink(small);
    ...
    public void changePriority(T item, double priority) {
        int index = 1;
        while (!item.equals(contents[index].myItem) && index <= size) {
            index++;
        }
        if (index > size) {
            return;
